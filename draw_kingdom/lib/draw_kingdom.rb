  ...
  file_readers.each do |current_file_reader|
    ...
    all_simulations.each_with_index do |current_simulation, index|
      ...
      print "Simulation " + (index+1).to_s +  ": " + ('%.2f' % money_gained_avg.to_s) + " NIS, " +
                ('%.2f' % succeeded_simulations_avg.to_s) + "%, Interest (Tsua'a) in percentage: " + simulation_record_manager.interest_on_money.to_s + ",Today: " + record_for_todays_team.team_object.team_name + "\n"
